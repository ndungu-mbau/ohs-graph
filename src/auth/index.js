...
router.post('/login', async (req, res) => {
  ...
  if(!user){
    ...
    const div = await collections["division"].findOne({ where: {
      id: department
    }})
    const dept = await collections["department"].findOne({ where: {
      id: department
    }})
    const token = jwt.sign({ id, name, department, type, phone }, SECRET)
    res.cookie('token', token)
    return res.json({ ok: true, token, user: { id, name, department: (div || dept), type: Object.assign(role, { permissions: role.permissions.split(",")}), phone }})
  }
})
